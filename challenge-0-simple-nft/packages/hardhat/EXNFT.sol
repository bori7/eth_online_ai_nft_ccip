 ...
 */
contract EXNFT is ERC721, ERC721URIStorage, ERC721Burnable, IAny2EVMMessageReceiver, ReentrancyGuard, OwnerIsCreator {
...
//    uint256 private _nextTokenId;
    euint256 private _nextTokenIdEnc;
    ...
    function mint(string memory uri) external onlyOnArbitrumSepolia {
//        uint256 tokenId = _nextTokenIdEnc++;
//        euint256 tokenIdEnc = _nextTokenIdEnc + FHE.asEuint256(1);
        euint256 tokenIdEnc =  FHE.add(_nextTokenIdEnc, FHE.asEuint256(1));
//        string memory uri = characters[tokenId % characters.length];
        _safeMint(msg.sender, FHE.decrypt(tokenIdEnc));
        _setTokenURI(FHE.decrypt(tokenIdEnc), uri);
    }
...
//        uint256 tokenId = _nextTokenId++;
        euint256 tokenIdEnc =  FHE.add(_nextTokenIdEnc, FHE.asEuint256(1));
        _nextTokenIdEnc = tokenIdEnc;
        string memory uri = characters[FHE.decrypt(tokenIdEnc) % characters.length];
        _safeMint(msg.sender, FHE.decrypt(tokenIdEnc));
        _setTokenURI(FHE.decrypt(tokenIdEnc), uri);
    }
